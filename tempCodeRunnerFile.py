...
# url = "rtsp://admin:Stellar11@169.254.62.241:554/Streaming/Channels/101"
# url1 = "http://10.87.77.222:8080/video"


# Open 2 videos
# caps = [
#     cv2.VideoCapture(url),
#     cv2.VideoCapture(url1),
# ]
...
frame_count = 0
skip_rate = 6   # detect (speed boost)
...
while True:
    ...
    # --- Show both input videos side by side ---
    disp_frames = []
    for f in frames:
        if f is None:
            f = np.zeros((360, 640, 3), dtype=np.uint8)  # black frame placeholder
        disp_frames.append(f)

    # 
    row_display = np.hstack(disp_frames)
    cv2.imshow("Input Videos (Side by Side)", row_display)
